...
## 重要：we_analyzer.py との関係について

本仕様書は **we_playbook.py** の技術仕様を記述したものですが、**we_analyzer.py** も本仕様書を基準としています。ただし、we_analyzer.py には以下の重要な相違点があります：

### we_analyzer.py の主な相違点

#### 1. 出力構成の簡略化
- **we_playbook.py**: 4シート出力（shortterm, longterm, monthly_trends, LatestIndividuals）
- **we_analyzer.py**: 2シート出力（monthly_trends, latest_individuals）のみ
  - shortterm・longterm シートは出力せず、詳細時系列分析に特化

#### 2. 列名の簡略化
- 全ての列名をユーザーフレンドリーな形式に変更
- 例：`__person__` → `person`、`Level_A` → `level`、`C_stability` → `stability_6`
- 詳細は [5.1 monthly_trends シート仕様](#5-monthly_trends-シート仕様) を参照

#### 3. 列名の typo
- `flag_constant_6m` → `flag_consistant_6m`（互換性維持のため意図的に保持）

#### 4. 追加された指標列
- `E_mean_3`: 3ヶ月平均
- `E_std_18`: 18ヶ月標準偏差
- `E_slope_6_std_12`: 正規化傾き（E_slope_6 ÷ E_std_12）
- 詳細は [5.2.7 多層統計指標列](#527-多層統計指標列) を参照

#### 5. 特性信頼度の数値化
- `trait_strength_conf_V/D/A`、`trait_weakness_conf_V/D/A` を数値（0.0-1.0の支持率）として出力
- 各Wave時点での累積計算（Expanding）を実施
- 詳細は [5.2.6 特性評価列](#526-特性評価列) を参照

#### 6. slope3m_pattern の挙動
- 個人ごとに1つの最終パターンのみを計算（Option B実装）
- monthly_trends では同一個人の全Wave行に同じパターンを表示
- 詳細は [4.3.1 slope3m_pattern](#431-slope3m_pattern長期推移パターン) を参照

**📌 本仕様書の読み方**
- 基本的なアルゴリズムと概念は we_playbook.py と共通
- 各セクションの「補足説明」に we_analyzer.py 固有の挙動を記載
- we_analyzer.py 実装者は、補足説明を特に注意して参照すること
...
**補足説明：we_analyzer.py の出力構成**
- **we_analyzer.py** では、簡略化のため **2シート構成**（monthly_trends、latest_individuals）のみを出力
- shortterm・longterm シートは出力されない（必要に応じて monthly_trends から集計可能）
- この仕様は、詳細な時系列分析と最新状態確認に特化した運用を想定
...
| 定数名 | 値 | 用途 |
|--------|-----|------|
| `TREND_SLOPE_POS` | 0.5 | 中期上昇トレンド判定の傾き閾値（E_slope_6 用） |
| `TREND_SLOPE_NEG` | -0.5 | 中期低下トレンド判定の傾き閾値（E_slope_6 用） |
| `TREND_SLOPE_STD_MIN` | 0.2 | 正規化傾き `E_slope_6_std_12` がほぼ 0 の場合のノイズ除去閾値 |
...
| `BIG_CHANGE_PERSONAL_Z` | 2.0 | 個人基準 big_change（|ΔE| が個人内 2σ 以上）判定閾値 |
...
**補足説明：**
- **we_analyzer.py における列名の相違**: we_analyzer.py では `flag_consistant_6m`（typo含む）として実装されている。これは意図的なスペルミスではなく、既存コードとの互換性維持のため。機能は同一（6ヶ月以上V/D/A/Eが完全不変の場合にTRUE）。
...
**補足説明：we_analyzer.py における slope3m_pattern の挙動**
- we_analyzer.py では、slope3m_pattern は**個人ごとに1つの最終パターンのみ**を計算（Option B実装）
- monthly_trends シートでは、この最終パターンが同一個人の全Wave行に繰り返し表示される
- つまり、Wave進行に伴う累積的なパターン変化（expanding）は記録せず、最新データに基づく全期間パターンのみを提供
- この実装により、各Wave時点での過去パターンの変遷ではなく、最終的な長期推移の特徴のみを把握できる
...
**補足説明：we_analyzer.py における列名の規則**

we_analyzer.py では、全ての列名をより簡潔でユーザーフレンドリーな形式に変更している。これは出力の可読性向上とデータ分析の利便性向上のための措置。列の意味と値は仕様と同一。

主要な列名変更：

| 仕様書での列名 | we_analyzer.py での列名 | 変更の意図 |
|---------------|------------------------|----------|
| `__person__` | `person` | アンダースコア除去（内部変数表記からユーザー向け表記へ） |
| `__wave__` | `wave` | アンダースコア除去 |
| `vigor_rating` | `vigor` | _rating サフィックス除去（簡潔化） |
| `dedication_rating` | `dedication` | _rating サフィックス除去 |
| `absorption_rating` | `absorption` | _rating サフィックス除去 |
| `Engagement` | `engagement` | 小文字化（統一性） |
| `Level_A` | `level` | サフィックス除去（レベル分類は1種類のみのため） |
| `Trend_B_base` | `trend_base` | プレフィックス除去、小文字化 |
| `Trend_B_recent` | `trend_recent` | プレフィックス除去、小文字化 |
| `Trend_B_refined` | `trend_refined` | プレフィックス除去、小文字化 |
| `ChangeTag` | `big_change` | より直感的な名称へ |
| `C_stability` | `stability_6` | プレフィックス除去、期間を明示（6ヶ月） |
| `C_stability_long` | `stability_12` | プレフィックス除去、期間を明示（12ヶ月） |
| `C_short_strength` | `short_strength` | プレフィックス除去 |
| `C_short_weakness` | `short_weakness` | プレフィックス除去 |
| `C_mid_strength` | `mid_strength` | プレフィックス除去 |
| `C_mid_weakness` | `mid_weakness` | プレフィックス除去 |
| `C_trait_strength` | `trait_strength` | プレフィックス除去 |
| `C_trait_weakness` | `trait_weakness` | プレフィックス除去 |

**命名規則の方針：**
- 内部変数的なアンダースコア（`__`）を除去
- 冗長なプレフィックス・サフィックスを削除
- 曖昧でない限り最短形式を採用
- 小文字とアンダースコアで統一（Pythonの慣例に準拠）
...
| 列名 | 型 | 説明 |
|------|----|------|
| `trend_base` | string | 中期トレンドの基礎区分。`E_slope_6` と正規化傾き `E_slope_6_std_12` に基づき `上昇中` / `低下中` / `安定` / `未評価` を判定する。 |
| `trend_recent` | string | **短期トレンド（変化量の大きさと連続性）**。`E_delta_1` および `E_delta_1_prev` に応じて `TREND_RECENT_DELTA = 3.0` を用いて、`連続上昇` / 急上昇 / 上昇` / `横ばい` / `低下` / 急落 /連続低下` を判定する。 |
| `trend_refined` | string | trenad_base と trend_recent（3区分）およびモメンタム等を統合した解釈ラベル。`上昇加速` / `上昇継続` / `上昇期待` / `低下加速` / `低下継続` / `低下危機` / `回復` / `復活` / `回復期待` / `低下懸念` / `低下警戒` / `安定維持` / `未評価` など。 |
| `big_change` | string | **個人基準の大きな変化**。`E_std_12` が正であり、`|E_delta_1| / E_std_12 ≥ BIG_CHANGE_PERSONAL_Z` を満たす場合に `変化大`、それ以外は空文字。 |
| `big_change_abs` | string | **組織基準の大きな変化**。`|E_delta_1| ≥ CHANGE_TAG_THRESHOLD` を満たす場合に `変化大`、それ以外は空文字。急上昇／急落判定とも整合。 |
...
| `trait_strength_conf_V` | 数値(0.00) | 特性強み・活力の支持率（Wave時点での累積） |
| `trait_strength_conf_D` | 数値(0.00) | 特性強み・熱意の支持率（Wave時点での累積） |
| `trait_strength_conf_A` | 数値(0.00) | 特性強み・没頭の支持率（Wave時点での累積） |
| `trait_weakness_conf_V` | 数値(0.00) | 特性弱み・活力の支持率（Wave時点での累積） |
| `trait_weakness_conf_D` | 数値(0.00) | 特性弱み・熱意の支持率（Wave時点での累積） |
| `trait_weakness_conf_A` | 数値(0.00) | 特性弱み・没頭の支持率（Wave時点での累積） |
| `Recovery Rate` | 数値(0.00) | 回復率（recovery/fall） |
| `Fall Rate` | 数値(0.00) | 転落率（fall/観測月数） |

**補足説明：trait_*_conf_V/D/A**
- これらの列は各Wave時点でのExpanding（累積）計算による特性信頼度を示す
- longtermシートの `Long_trait_*_conf_V/D/A` とは異なり、monthly_trendsでは各Wave時点までの特性カウントに基づく
- 計算方法：
  - 各Wave時点までの `C_trait_strength` / `C_trait_weakness` に出現した各次元（活力/熱意/没頭）の回数をカウント
  - 支持率 = その次元の出現回数 ÷ 全次元の総出現回数
  - 例：Wave 1-12で「活力」が8回、「熱意」が10回、「没頭」が6回出現した場合
    - `trait_strength_conf_V` = 8/24 = 0.333
    - `trait_strength_conf_D` = 10/24 = 0.417
    - `trait_strength_conf_A` = 6/24 = 0.250
- 評価条件を満たさないWaveではNaNとなる（履歴不足、High/Low比率不足など）
...
| `E_momentum_6` | 数値(0.00) | 6ヶ月モメンタム（直近6ヶ月平均 - 前6ヶ月平均） |
| `E_delta_1` | 数値(0.00) | 前月差分 |
| `E_delta_1_prev` | 数値(0.00) | 前々月差分 |
| `E_mean_3` | 数値(0.00) | 3ヶ月平均（直近3ヶ月のEngagementの単純平均） |
| `E_mean_6` | 数値(0.00) | 6ヶ月平均 |
| `E_std_6` | 数値(0.00) | 6ヶ月標準偏差 |
| `E_std_12` | 数値(0.00) | 12ヶ月標準偏差 |
| `E_std_18` | 数値(0.00) | 18ヶ月標準偏差（直近18ヶ月のEngagementの標準偏差） |
| `E_iqr_6` | 数値(0.00) | 6ヶ月IQR（四分位範囲） |
| `E_slope_12` | 数値(0.00) | 12ヶ月Theil-Sen傾き |
| `E_slope_6` | 数値(0.00) | 6ヶ月Theil-Sen傾き |
| `E_slope_6_std_12` | 数値(0.00) | 正規化傾き（E_slope_6 ÷ E_std_12）。12ヶ月の変動幅に対する6ヶ月傾きの相対的な強さを示す |
| `E_delta_1_std_12` | float | 1ヶ月差分の個人標準化変化量。`E_delta_1 / E_std_12` として計算する。`E_std_12 <= 0` の場合は `NaN`。`big_change` 判定に利用する。 |
| `E_accel_6` | 数値(0.00) | 6ヶ月加速度（E_slope_6の前月差分） |

**補足説明：**
- `E_mean_3`: 短期的な水準変化を平滑化して把握するための指標
- `E_std_18`: より長期（1年半）の変動性を評価するための指標。C_stability_long（12ヶ月）よりもさらに長期の安定性分析に使用可能
- `E_slope_6_std_12`: 変動幅で正規化することで、個人間での傾きの強さを比較可能にする。例：std=2.0の人のslope=0.4は、std=4.0の人のslope=0.8と同等の相対的変化
