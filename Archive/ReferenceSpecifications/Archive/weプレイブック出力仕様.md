...
| `InterventionPriority` | 整数 | 介入優先度スコア（優先行動のソートキー） |
...
- トレンド別の基礎点: 低下加速=10, 低下危機=8, 悪化=6, 低下継続=4, 低下懸念=2, 低下警戒=1, 上昇加速=5, 復活=3, 回復=1（上記以外は0）
- `Level_A` が Critical/Low なら +1
- `E_delta_1` の絶対値が `CHANGE_TAG_THRESHOLD`(=6.0) 以上かつ負方向なら +3、正方向なら +2
- 合計値を整数で出力。値が高い順に1on1等の介入を優先
- C_stability が高いメンバーはコミュニケーション頻度を高めるなど運用上の留意点と合わせて利用
...
**slope3m_pattern**：
- `slope_3m` の直近12Wave（`SLOPE_PATTERN_WINDOW=12`）を対象に、符号比率・平均・符号変化回数でルール判定
...
- 評価対象は履歴数≥6のメンバーのみ。さらに `pct_high`（または`pct_low`）が `_dynamic_level_ratio_threshold(total_records)` を満たさない場合は空欄
...
| | `Recovery Rate` | 数値(0.00) | Low→(Mid/High)転換回数 ÷ (Mid/High→Low転換回数) |
| | `Fall Rate` | 数値(0.00) | (Mid/High→Low転換回数) ÷ 観測月数 |
...
- `C_trait_strength/weakness` は 12ヶ月ローリング中央値に加え、履歴数≥6 のときのみ計算。High/Low 滞在比率が `_dynamic_level_ratio_threshold()` を下回る場合は空欄（閾値は履歴6ヶ月で0.75、履歴18ヶ月で約0.60に線形で緩和）
...
- `Recovery Rate = episodes_recovery_from_low ÷ episodes_fall_to_low`（分母0の場合は0）
- `Fall Rate = episodes_fall_to_low ÷ 観測月数（そのWave時点までの行数）`
...
- longterm シートの値と一致する
- `Recovery Rate`/`Fall Rate` も monthly_trends と同値（expanding最終値）を保持
