...
flowchart TD
    ...
    CheckChange -->|Yes| CheckSlope{E_slope_6<br/>> 0.5?}

    CheckSlope -->|No| NotMatch4[該当しない]
    CheckSlope -->|Yes| Result[上昇加速]

    Note1[上昇系:<br/>E_delta_1 > 2.0<br/>OR E_delta_1 >= 6.0<br/>OR 連続上昇]
    Note2[上昇トレンド:<br/>E_slope_6 > 0.5 AND<br/>E_slope_6_std_12 > 0.2<br/>OR<br/>E_slope_6_std_12 > 0.45]
...
flowchart TD
    ...
    Note2[低下トレンド:<br/>E_slope_6 < -0.5 AND<br/>E_slope_6_std_12 < -0.2<br/>OR<br/>E_slope_6_std_12 < -0.45]
...
flowchart TD
    ...
    CheckChange -->|No| NotMatch3[該当しない]
    CheckChange -->|Yes| Result[回復]

    Note1[上昇系:<br/>2.0 < E_delta_1 < 6.0<br/>OR E_delta_1 >= 6.0<br/>OR 連続上昇]
...
flowchart TD
    ...
    CheckChange -->|No| NotMatch3[該当しない]
    CheckChange -->|Yes| Result[低下危機]

    Note1[低下系:<br/>-6.0 < E_delta_1 < -2.0<br/>OR E_delta_1 <= -6.0<br/>OR 連続下降]
...
flowchart TD
    Start([判定開始]) --> CheckRecent{E_delta_1で<br/>上昇系?}
    CheckRecent -->|No| NotMatch1[該当しない]
    CheckRecent -->|Yes| CheckBase{E_slope_6で<br/>安定 or 上昇中?}

    CheckBase -->|No| NotMatch2[該当しない]
    CheckBase -->|Yes| Result[上昇期待]

    Note1[上昇系:<br/>E_delta_1 > 2.0<br/>OR E_delta_1 >= 6.0<br/>OR 連続上昇]
    Note2[安定 or 上昇中:<br/>明確な低下トレンドなし]
...
flowchart TD
    Start([判定開始]) --> CheckRecent{E_delta_1で<br/>低下系?}
    CheckRecent -->|No| NotMatch1[該当しない]
    CheckRecent -->|Yes| CheckBase{E_slope_6で<br/>安定 or 低下中?}

    CheckBase -->|No| NotMatch2[該当しない]
    CheckBase -->|Yes| Result[低下警戒]

    Note1[低下系:<br/>E_delta_1 < -2.0<br/>OR E_delta_1 <= -6.0<br/>OR 連続下降]
    Note2[安定 or 低下中:<br/>明確な上昇トレンドなし]
...
|------|------|
| 未評価 | データ件数 < 3 |
| 上昇中 | (E_slope_6 > 0.5 **AND** E_slope_6_std_12 > 0.2) **OR** (E_slope_6_std_12 > 0.45) |
| 低下中 | (E_slope_6 < -0.5 **AND** E_slope_6_std_12 < -0.2) **OR** (E_slope_6_std_12 < -0.45) |
| 安定 | 上記以外 |
