...
<!-- 最終更新: 2026-06-02（stability_6 を個人内基準（E_std_6 P90/P75）に変更・判定保留追加、direction_6_p90「横ばい」→「方向変化なし」、§11 に stdNorm 設計根拠を追記） -->
...
### なぜ std_6 ではなく std_12（stdNorm）を正規化分母に使うか

`E_std_6` は `stability_6` でスコアの**変動幅そのもの**を測る被測定値（シグナル）である。
これを `E_delta_1` の正規化分母にも使うと、高変動期の変化量が過小評価されるという逆効果が生じる。

```
高変動期の例（E_std_6 = 5.0, E_std_12 = 2.0）:
  E_delta_1 = +4.0 のとき
  std_6 で正規化 → 4.0 / 5.0 = 0.8σ  tier 0（検出されない）
  std_12 で正規化 → 4.0 / 2.0 = 2.0σ  tier 2（検出される）
```

高変動期に `stability_6 = "不安定"` が出ているとき、同じ期間の `E_delta_1` の異常も捉えたいのに、std_6 を分母にするとその変化が「見えなくなる」。std_12 はより長い安定したベースラインであり、一時的な変動に影響されにくい参照軸として機能する。

また、`E_std_6` は 6 件で計算でき更新頻度が高いため、短期的な状態を反映しすぎる。`E_delta_1_std_12` の意図は「この月の変化量は、その人の**長期的な通常値域**からどれほど外れているか」であり、std_12 のほうが概念的に適合する。

**カラム名の注意**: `E_delta_1_std_12`・`E_slope_6_std_12` というフィールド名は「std_12 を優先的に使う」の意であり、データ数が 6–11 件のときは実際には std_6 が分母になる（stdNorm フォールバック）。厳密には `E_delta_1_stdNorm` と呼ぶべきものである。
