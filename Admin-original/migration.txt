//
// Migration script to update existing RatingSS data with new analysis fields
//

function migrateRatingSSToNewFormat() {
  console.log("Starting migration of RatingSS data to new format...");
  ...
  if (colYear === -1 || colMonth === -1 || colMailAddress === -1 || colEngagement === -1) {
    throw new Error("Required columns not found in rating sheet");
  }

  // Group data by mail address
  const userDataMap = {};
  rows.forEach((row, index) => {
    ...
    userDataMap[address].rows.push(row);
    userDataMap[address].rowIndices.push(index + 2); // +2 because header is row 1 and we're 0-indexed
  });

  const addresses = Object.keys(userDataMap);
  console.log(`Total unique users: ${addresses.length}`);

  let processedCount = 0;

  // Process each user
  addresses.forEach(address => {
    const userData = userDataMap[address];
    const userRows = userData.rows;

    // Sort by year and month
    const sortedData = userRows.map((row, idx) => ({
      row: row,
      originalIndex: userData.rowIndices[idx]
    })).sort((a, b) => {
    ...
    // Prepare data for analyzeEngagement
    const dataForAnalysis = [header].concat(sortedData.map(item => item.row));

    // Process each row for this user
    sortedData.forEach((item, idx) => {
      const rowData = item.row;
      const sheetRowIndex = item.originalIndex;

      // Get data up to and including current row
      const dataUpToNow = [header].concat(sortedData.slice(0, idx + 1).map(x => x.row));

      // Analyze engagement for this point in time
      const analysisResult = analyzeEngagement(dataUpToNow);

      if (analysisResult && Object.keys(analysisResult).length > 0) {
        // Update the row with analysis results
        updateRowWithAnalysisResults(ratingSheet, sheetRowIndex, analysisResult);
        processedCount++;
  ...
  console.log(`Migration completed. Total rows updated: ${processedCount}`);
}
...
function updateRowWithAnalysisResults(sheet, rowIndex, results) {
  ...
  const values = [
    ...
    formatValue(results.V_deltaP10),
    formatValue(results.D_deltaP10),
    formatValue(results.A_deltaP10),
    formatValue(results.V_deltaP90),
    formatValue(results.D_deltaP90),
    formatValue(results.A_deltaP90),
    formatValue(results.V_deltaZ),
    formatValue(results.D_deltaZ),
    formatValue(results.A_deltaZ),
    formatValue(results.V_slopeP10),
    formatValue(results.D_slopeP10),
    formatValue(results.A_slopeP10),
    formatValue(results.V_slopeP90),
    formatValue(results.D_slopeP90),
    formatValue(results.A_slopeP90),
    formatValue(results.V_slopeZ),
    formatValue(results.D_slopeZ),
    formatValue(results.A_slopeZ),
    formatValue(results.E_momentum_3),
    formatValue(results.E_delta_1),
    formatValue(results.E_delta_1_prev),
    formatValue(results.E_mean_6),
    formatValue(results.E_std_6),
    formatValue(results.E_slope_12),
    formatValue(results.E_slope_6),
    formatValue(results.E_accel_6),
...
// Copy all analysis functions from evaluate.txt below
// --- Configuration Parameters ---
const TREND_SLOPE_POS = 0.35;
const TREND_SLOPE_NEG = -0.35;
const TREND_MOMENTUM_STRONG = 1.5;
const TREND_DELTA_STRONG = 5.0;
const TREND_DELTA = 1.0;
const LEVEL_THRIVING = 43;
...
function analyzeEngagement(data) {
  ...
  ENGAGEMENT_RESULT_FIELDS.forEach(field => {
    ...
    if (!hasMidHistory && MID_DEPENDENT_STRING_FIELDS.has(field)) {
      ...
      results[field] = Number.isFinite(value) ? value : 0;
    } else {
      results[field] = value !== undefined && value !== null ? value : "";
    }
  });

  // Add V/D/A specific statistics
  ["V_deltaP10", "D_deltaP10", "A_deltaP10",
   ...
   "V_slopeZ", "D_slopeZ", "A_slopeZ"].forEach(field => {
    const value = latestMetric[field];
    results[field] = Number.isFinite(value) ? value : "";
  });
