...
| 33–37 | 統計指標 | Report | E_delta_1, E_delta_1_prev, E_delta_1_std_12, E_slope_6, E_slope_6_std_12 |
| 38 | E_slope_3m | Report | 3期OLS傾き（旧・末尾から E_slope_6_std_12 と V_delta_1 の間へ移動） |
| 39 | direction_6_p90 | Report | 中期方向（上昇/下降/横ばい/判定保留）。個人内変動指標（過去窓 D6 の P90 を閾値） |
| 40 | volatility_6_p90 | Report | 中期波動（波動あり/波動なし/判定保留）。個人内変動指標（過去窓 R6 の P90 + 符号反転≥3） |
...
├──────────────────────┼───────────────────────┼──────────┤
│ volatility_6_p90     │ "波動あり"（方向不問）│ neg +2   │
├──────────────────────┼───────────────────────┼──────────┤
...
| **stability_6** | `"不安定"`（E_std_6 >= 3.3, 組織内SD基準）は変動が大きく予測困難な状態。方向不問で neg +1（上昇局面の不安定も負方向シグナルとして扱う。従来の E_delta_1 符号による pos/neg 振り分けは廃止）。 |
| **volatility_6_p90** | `"波動あり"`（個人内基準の反復的変動：過去窓 R6 の P90 超＋符号反転≥3）は方向不問で neg +2。stability_6（組織内SD基準）とは寄与が異なるため両方発火を許容（最大 neg +3）。**Playbook/we_analyzer.py の介入必要度ロジックと完全同期。** |
...
*最終更新: 2026-06-01（rating2 を48列に拡張：direction_6_p90 / volatility_6_p90 追加・E_slope_3m 移動。介入必要度に volatility_6_p90 +2 / stability_6 方向不問 +1。`rebuildAllRating2()` 追加）*
