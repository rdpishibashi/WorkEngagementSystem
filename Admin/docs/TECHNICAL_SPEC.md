...
| **volatility_6_p90** | `"波動あり"`（個人内基準の反復的変動：過去窓 R6 の P90 超＋符号反転≥3）は方向不問で neg +2。stability_6（個人内基準）とは寄与が異なるため両方発火を許容（最大 neg +3）。**Playbook/we_analyzer.py の介入必要度ロジックと完全同期。** |
...
*最終更新: 2026-06-02（stability_6 を個人内基準（E_std_6 P90/P75）に変更・「やや不安定」追加・「判定保留」追加。介入必要度に「やや不安定」→ neg +1 を追加。Playbook/we_analyzer.py と完全同期）*
