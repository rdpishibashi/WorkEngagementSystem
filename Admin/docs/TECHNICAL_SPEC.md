...
│                      │ |value| > 1.5         │ +4       │
├──────────────────────┼───────────────────────┼──────────┤
│ トレンド乖離         │ E_slope_6≥0 &         │ neg +1   │
│（短期 vs 中期）      │ E_slope_3m < -0.2     │          │
│                      │ E_slope_6≤0 &         │ pos +1   │
│                      │ E_slope_3m > 0.2      │          │
├──────────────────────┼───────────────────────┼──────────┤
...
| **trend_recent** | `急落/急上昇`（E_delta_1 ≥ 6）は緊急性が高いため +2、`連続下降/連続上昇`（2期連続で ±2超）は継続性があるため +1。`下降/上昇`（moderate）は trend_base で既に拾われるため加点しない。 |
...
| **トレンド乖離** | 中期（E_slope_6）と短期（E_slope_3m）の方向が逆転している場合に転換点のシグナルとして加点。閾値 0.2 は統計的ノイズを除外する最小ラインとして設定。 |
...
#### TREND_SLOPE = 0.2 — トレンド乖離の閾値

`E_slope_3m = (E[t] - E[t-2]) / 2` の単位は**生スコアの点数/月**（stdNorm で割らない非標準化値）。

```javascript
// 発火条件:
neg +1 :  E_slope_6 >= 0  AND  E_slope_3m < -0.2   // 中期横ばい以上 + 直近2ヶ月で下降
pos +1 :  E_slope_6 <= 0  AND  E_slope_3m >  0.2   // 中期横ばい以下 + 直近2ヶ月で上昇
```

| 項目 | 説明 |
|------|------|
| `0.2 点/月` | 2ヶ月間で 0.4 点の変化に相当。これ未満は「方向転換とは言えない微小な揺れ」として無視 |
| `E_slope_6 >= 0` | 中期が横ばい or 上昇中。この状態で直近が下降なら転換の予兆 |
| `E_slope_6 <= 0` | 中期が横ばい or 下降中。この状態で直近が上昇なら回復の予兆 |
| 非標準化 | E_slope_3m は個人の変動幅に依らず同じ閾値を適用。変動の大きい人には感度が低く、変動の小さい人には感度が高い |

> **DELTATIERS / SLOPETIERS との違い**: E_slope_3m は短期方向転換の早期検知に特化したシグナルであり、個人内標準化をしない代わりに処理が単純で、直近 3 点の生データから即座に算出できる。
