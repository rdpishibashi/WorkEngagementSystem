...
**戻り値**：33のプロパティを持つレーティングオブジェクトの配列。RatingSS のカラムインデックスを名前付きフィールド（year, month, day, date, address, engagement, vigor, dedication, absorption, level, trend_base, trend_recent, trend_refined, big_change, stability_6, strength/weakness short/mid, 統計指標, **direction_6_p90, volatility_6_p90**）にマッピングする。`ColumnRating*` 定数（Globals.gs）は RatingSS の新列順に合わせ E_slope_3m=24 / direction_6_p90=25 / volatility_6_p90=26 / V_delta_1 以降=27… へ再付番済み。
...
1人あたり**1行**（rating2シート用の全48列）を生成する。

主な処理：
1. `calculateInterventionPriority(rating)` を呼び出し → `{neg, pos}`を取得
2. 共通カラム、生スコア、分析フィールド、介入必要度、統計指標、個人内変動指標（direction_6_p90 / volatility_6_p90）を組み合わせた46要素の配列を構築（E_slope_3m を E_slope_6_std_12 と V_delta_1 の間に配置）
...
スコアリングテーブルの詳細はTECHNICAL_SPEC.md「介入必要度の算出」セクションを参照。`stability_6 == "不安定"` → neg +1（方向不問）、`volatility_6_p90 == "波動あり"` → neg +2（方向不問）。**Playbook/we_analyzer.py の `calculate_intervention_priority` と完全同期。**
...
rating2シートの48列のヘッダーを定義する定数配列。`ensureRating2Headers()`でヘッダーの整合性を保証するために使用される。`E_slope_3m` を `E_slope_6_std_12` と `V_delta_1` の間へ移動し、その直後に `direction_6_p90`, `volatility_6_p90` を配置（46→48列）。
...
EngagementMasterSS の rating2 シートを**全データ再構築**する（`maintenance.gs`）。列構成変更（direction_6_p90 / volatility_6_p90 追加・E_slope_3m 移動）後、`updateMaster()` が当月分しか追記しないために過去月の行が旧列構成のまま残る問題を解消する。
...
**処理フロー：**
1. RatingSS rating シート（`recalculateRatingSheet()` 実行済み）から全 (year, month) を時系列収集
2. 各波形で `getRatingsData()` + `computeFlagConstant6mMap()` + `createRating2MasterToBeAdded()` を呼び、全 rating2 レコードを新レイアウト（48列・新介入優先度）で構築
3. `ensureRating2Headers()` 後、既存データ行を全消去して一括書き込み
4. `updateOrganizationData()` で `current_*` を更新

**注意**：rating2 を全書き換えする破壊的操作（実行前バックアップ推奨）。rating シートのみ参照し、rating / evaluation / comment は変更しない。デプロイ順序は TECHNICAL_SPEC.md「保守：rating2 の列構成変更・全データ再構築」を参照。
...
**比較フィールド**（19項目）：level, trend_base, trend_recent, trend_refined, big_change, stability_6, direction_6_p90, volatility_6_p90, E_delta_1, E_delta_1_prev, E_delta_1_std_12, E_slope_6, E_slope_6_std_12, V/D/A_delta_1, V/D/A_slope_6。
...
*最終更新: 2026-06-01（rating2 48列化：direction_6_p90 / volatility_6_p90、ColumnRating* 再付番、介入必要度の volatility +2 / stability 方向不問 +1、`rebuildAllRating2()` 追加）*
