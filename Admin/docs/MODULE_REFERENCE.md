...
| ファイル | 行数 | 目的 |
|---|---|---|
| `Globals.gs` | 154 | グローバル変数、スプレッドシート参照、カラム定数 |
| `master_updater.gs` | 46 | メインオーケストレーター — `updateMaster()` |
...
| `member_management.gs` | 115 | メンバーリスト、組織属性更新 |
| `maintenance.gs` | 388 | メンテナンスユーティリティ — ヘッダー更新、データ削除、メールアドレス修復 |
...
```
ConfigurationSS → セルC3–C11を読み取り → 9つのスプレッドシートをオープン
```
...
7. `updateOrganizationData()` — 全マスタシートの現在の組織属性を更新
...
**行ごとのロジック：**
- **メンバーが見つかり、休職中**（`leave === "Y"`）：全組織フィールドをクリア
- **メンバーが見つかり、在籍中**：各フィールドを比較し、1つでも差異があれば更新キューに追加
...
年月がハードコードされた便利関数。`deleteMonthData()`を呼び出す。
...
### `deleteMonthFromSheet(sheet, year, month, sheetName)`
...
3. 最新のマッチング行に対して`rebuildIndividualSheetInternal()`を呼び出す
4. 各メンバーのエンゲージメントスコアをログ出力

### `updateMasterSheetAttributes()`

`alternativeName`を名前フィールドに使用して、4つのマスタシートすべてのメンバー属性を一括更新する。

`updateOrganizationData()`（差分ベース・行単位）とは異なり、この関数は：
- O(1)アクセス用のメンバー検索マップを構築
- シート全体をメモリに読み込み、インプレースで変更、1回のオペレーションで書き戻す
- 1ベースのカラムインデックスを使用（レガシーコード）

### `recoverInvalidMailAddress()`

不正なメールアドレスを全シート（Answer, Rating, Comment）で修正し、個人シートを再構築する。
...
**処理手順：**
1. AnswerSheetでメールアドレスを修正
2. RatingSheetでメールアドレスを修正
3. 修正後のアドレスで個人シートを再構築
4. 旧個人シート（不正な名前のもの）を削除
5. CommentSheetでメールアドレスを修正

### `rebuildIndividualSheetInternal(address, startDate, period, ratingRowNumber)`

RatingSS 内の個人シートを再構築するコア関数。

```javascript
function rebuildIndividualSheetInternal(address, startDate, period, ratingRowNumber = null) → Object
```
...
### `updateEngagementStatus(sheet, engagementStatus, row)`
