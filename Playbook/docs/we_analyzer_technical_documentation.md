...
> 最終更新: 2026-05-30（個人内変動指標 direction_6/volatility_6/change_1m/acceleration_6/intervention_priority_6 を追加、E_accel_6→acceleration_6 統合）  
...
7. [出力指標辞典](#7-出力指標辞典)
   ...
   - [7.12 個人内変動指標群](#712-個人内変動指標群directionvolatilitychangepriority_6)
8. [閾値一覧](#8-閾値一覧)
9. [判定指標の詳細](#9-判定指標の詳細)
   - [9.1 level](#91-level)
...
| 介入優先度 | `intervention_priority_neg`, `intervention_priority_pos`, `intervention_priority_6` |
| 個人内変動 | `direction_6`, `volatility_6`, `change_1m`, `acceleration_6` |
...
19. add_personal_variability_features()
      └── direction_6, volatility_6, change_1m
          （acceleration_6 は step 5 で算出済み。trend_* とは独立）

20. calculate_intervention_priority_6()
      └── intervention_priority_6（direction_6 × volatility_6 × 現在水準）

21. monthly_trends / latest_individuals を構成

22. _write_excel_output()
...
| 関数名 | 説明 |
|--------|------|
| `add_personal_variability_features(df)` | direction_6, volatility_6, change_1m を算出（trend_* と独立） |
| `calculate_intervention_priority_6(row)` | intervention_priority_6（カテゴリ）を判定 |
| `_mad_scaled_sigma(arr)` | 1.4826×MAD による頑健標準偏差 |
...
6ヶ月安定性。カテゴリは `不変`, `安定`, `やや安定`, `不安定`。履歴数が2以下なら空文字。

| 優先 | 値 | 条件 |
|------|-----|------|
| 1 | `不変` | E/V/D/A の6ヶ月 range がすべて ≤ 1e-6 |
...
| `acceleration_6` | $E\_slope\_6(t) - E\_slope\_6(t-1)$ | 6ヶ月傾きの変化量（前回傾きが無ければ 0.0、履歴≤2は NaN）。0–54 尺度。`add_multiscale_features()` で算出 |
| `V_slope_6` / `D_slope_6` / `A_slope_6` | Theil-Sen（直近6点以内） | 各次元の6ヶ月ロバスト傾き |

> **Note**: 旧 `E_accel_6`（出力されない内部列）は本指標 `acceleration_6` に統合され、出力列となった。
...
### 7.12 個人内変動指標群（direction / volatility / change / priority_6）

`add_personal_variability_features()` および `calculate_intervention_priority_6()` が算出する独立指標群。
既存の階層的判定（`trend_base` / `trend_recent` / `trend_refined`）とは**独立**に、0–54 尺度の `engagement` そのものに対して各 wave 時点で因果的（causal）に算出される。

**設計思想**: 標準偏差ベースの `stability_6` や標準化傾き `E_slope_6_std_6`（= `E_slope_6 / E_std_6`）は方向性と変動性を混在させる。介入判断のため、**方向（direction）／波動（volatility）／短期変化（change）／現在水準（level）** を分離する。

**個人内通常変動幅 σ**: 個人の全履歴 `engagement` に対する expanding（causal inclusive）な
$$\sigma = \max(1.4826 \times MAD,\ \text{DIR6\_SIGMA\_FLOOR}=2.25)$$
MAD は中央値絶対偏差。閾値はすべて 0–6 尺度の提案値を 0–54 尺度へ線形換算（×9）したもので、判定結果は 0–6 版と数学的に同一。

#### `direction_6`（中期方向）

カテゴリ: `下降`, `上昇`, `横ばい`, `判定保留`。有効履歴 < 6（`E_std_6` が NaN）なら `判定保留`。

- `slope_6 = E_slope_6`（既存 Theil-Sen 傾き）、6ヶ月予測変化量 $D_6 = 5 \times slope\_6$
- 閾値 $T_{dir} = \max(0.5\sigma,\ 2.70)$
...
カテゴリ: `高`, `通常`, `判定保留`。**`direction_6 == 横ばい`（明確な方向性が無い）場合にのみ波動を評価**し、それ以外は `通常`。きれいな急上昇/急下降はレンジが大きくても波動には入れない（波動概念＝方向で説明できない上下変動）。

- 直近6点に傾向線（slope=`E_slope_6`, 切片=$\text{median}(y - slope \cdot x)$）を引き、残差SD $R_6 = SD(y - \hat{y})$、レンジ $Range_6 = \max - \min$
- $T_{wave} = \max(0.75\sigma,\ 3.15)$、$Range_{thr} = \max(1.5\sigma,\ 6.30)$

| 条件（`横ばい` 時のみ） | 判定 |
|------|------|
| $R_6 \geq T_{wave}$ または $Range_6 \geq Range_{thr}$ | `高` |
| それ以外 | `通常` |

#### `change_1m`（短期変化）

カテゴリ: `低下`, `上昇`, `横ばい`。直近1ヶ月差分 `E_delta_1` を、個人の過去差分の頑健SDベース閾値で判定。

- $T = \max(1.4826 \times MAD(\text{過去差分}),\ 2.70)$（過去差分は初月の擬似値 0 を除外）

| 条件 | 判定 |
|------|------|
| $E\_delta\_1 \leq -T$ | `低下` |
| $E\_delta\_1 \geq T$ | `上昇` |
| それ以外（初月含む） | `横ばい` |

#### `intervention_priority_6`（統合介入優先度・カテゴリ）

カテゴリ: `最優先`, `高`, `中`, `低`, `判定保留`。`direction_6` / `volatility_6` / 現在水準から判定。
現在水準は当該 wave の `engagement`: 低 (< 22.5) / 中 (22.5–31.5) / 高 (> 31.5)。

判定順（上から最初に一致。原則 下降 > 波動 > 上昇、低水準は格上げ）:
...
| 2 | `下降` | `高` |
| 3 | 波動`高` かつ 低水準 | `高` |
| 4 | `横ばい` かつ 低水準 | `高`（慢性的低エンゲージメント） |
| 5 | 波動`高` | `中` |
| 6 | `上昇` かつ 低水準 | `中` |
| 7 | `上昇` | `低` |
| 8 | それ以外（横ばい・中/高・通常） | `低` |
| — | `direction_6 == 判定保留` または engagement NaN | `判定保留` |

> 介入優先度の順序（下降＞波動＞上昇、低水準は格上げ）は実務的なリスク・トリアージ仮説であり、UWES 研究で直接検証された定理ではない。既存の `intervention_priority_neg/pos`（数値スコア）とは別系統の補助指標である。
...
| `STABILITY_STD_STABLE` | 1.0 | E_std_6 がこれ以下で「安定」候補 |
| `STABILITY_MOMENTUM_STABLE` | 0.5 | `|E_momentum_3|` がこれ以下で「安定」候補 |
| `STABILITY_STD_UNSTABLE` | 3.3 | E_std_6 がこれ以上で「不安定」 |
...
### 8.10 個人内変動指標（direction/volatility/change/priority_6）

0–6 尺度の提案閾値を 0–54 尺度へ ×9 換算した定数。係数（0.5σ 等）は無次元のため不変。
...
| `DIR6_SLOPE_COEF` | 0.5 | T_dir = max(0.5σ, floor) |
| `DIR6_WAVE_COEF` | 0.75 | T_wave = max(0.75σ, floor) |
| `DIR6_RANGE_COEF` | 1.5 | range_thr = max(1.5σ, floor) |
| `DIR6_D6_HORIZON` | 5 | D6 = 5×slope_6（6ヶ月予測変化量） |
| `CHANGE1M_FLOOR` | 2.70 | change_1m 判定閾値の下限（0.30×9） |
| `LEVEL06_LOW` | 22.5 | priority_6 の低水準上限（2.5×9） |
| `LEVEL06_HIGH` | 31.5 | priority_6 の高水準下限（3.5×9） |
...
- 傾き系: `E_slope_6`, `E_slope_12`, `E_slope_6_std_6`, `E_slope_6_std_12`, `acceleration_6`, `V_slope_6`, `D_slope_6`, `A_slope_6`
...
`calculate_intervention_priority(row)` が以下7項目のスコアを加算する。`intervention_priority_neg` と `intervention_priority_pos` を独立に算出。
...
### (2) E_delta_1 直近変化量（0–3点）

raw の `E_delta_1` 値で直接加点（trend_recent カテゴリを経由しない）。
...
| `2.0 <= E_delta_1 < 6.0` | — | +1 |
| `-6.0 < E_delta_1 <= -2.0` | +1 | — |
| `E_delta_1 >= 2.0` かつ `E_delta_1_prev >= 2.0`（連続変化加点） | — | +1 |
| `E_delta_1 <= -2.0` かつ `E_delta_1_prev <= -2.0`（連続変化加点） | +1 | — |

最大：2（急変）+ 1（連続）= 3点。連続変化加点は急変加点と同時に加算される。
...
| 条件 | neg | pos |
|------|-----|-----|
| `stability_6 == "不安定"` かつ `E_delta_1 < 0` | +1 | — |
| `stability_6 == "不安定"` かつ `E_delta_1 > 0` | — | +1 |

### (5) E_delta_1_std 段階スコア（0–4点）

`E_delta_1_std_12` を優先使用。NaN の場合は `E_delta_1_std_6` にフォールバック。符号で neg/pos を振り分ける。
...
### (8) flag_constant_6m 加点（neg のみ、0–4点）

`calculate_intervention_priority()` 実行後に `run()` 内で別途加算される。
...
|--------|----------------------------------------|
| `intervention_priority_neg` | 1+3+1+1+4+4+1 = **15** |
| `intervention_priority_pos` | 1+3+1+1+4+4+1 = **15** |

flag_constant_6m の FIX_SHIFTED 加点が入ると neg は最大 **19**。
...
`Prev_E_slope_6`, `E_min6_past`, `E_max6_past`, Z-score 系列などは内部計算だけに使われる（§11参照）。なお旧 `E_accel_6` は出力列 `acceleration_6` に統合された（§7.10）。
