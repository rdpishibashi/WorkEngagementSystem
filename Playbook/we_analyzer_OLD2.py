...
# ========== Constants (Updated: using absolute values) ==========
# Slope thresholds (absolute values)
...
TREND_DELTA = 1.0
TREND_RECENT_DELTA = 2.0      # trend_recent の上昇／低下判定閾値（先月比）

# V/D/A の short/mid strength/weakness 判定用の閾値
Z_VDA_THRESHOLD = 0.8         # 旧: Z_THRESHOLD, Z_POS/Z_NEG
SHORT_VDA_MIN_DELTA = 2.0     # 旧: SHORT_MIN_DELTA

# Personal change threshold
BIG_CHANGE_PERSONAL_Z = 2.0   # |E_delta_1| / E_std_12 の閾値（個人内 2σ、> で判定）
...
# Other constants
MID_MIN_RECORDS = 2  # 3ヶ月以上のデータで中期トレンドを計算
...
SLOPE12_THRESHOLD = 0.4             # 旧: SLOPE12_POS_MIN/NEG_MAX (absolute value)
SLOPE6_STD12_THRESHOLD = 0.2        # 旧: SLOPE6_STD12_POS_MIN/NEG_MAX (absolute value)
...
def apply_personal_trend_logic(df_in: pd.DataFrame) -> pd.DataFrame:
    ...
    # 履歴不足の人は傾き系指標を NaN に
    slope_cols = ["E_slope_6", "E_slope_12", "E_slope_6_std_12", "E_accel_6", "V_slope_6", "D_slope_6", "A_slope_6"]
    ...
    # ---- 統合トレンド (Trend_B_refined) を新仕様で実装 ----
    def _refine(row: pd.Series) -> str:
        ...
        # Priority 5: 上昇期待
        if (trend_recent in ["上昇", "急上昇", "連続上昇"] and
            trend_base in ["安定", "上昇中"]):
            return "上昇期待"

        # Priority 5: 低下警戒
        if (trend_recent in ["下降", "急落", "連続下降"] and
            trend_base in ["安定", "低下中"]):
            return "低下警戒"
...
def compute_slope3m_pattern(monthly_trends: pd.DataFrame) -> pd.DataFrame:
    ...
    for pid, g in monthly_trends.groupby(PERSON_COL):
        ...
        if N <= 3:
            ...
            # Step 2. Net Growth / Net Decline (絶対値使用)
...
def run(input_path: Path, output_path: Path, mid_window: int = 6):
    ...
    if dup_mask.sum() > 0:
        duplicates = use[use.duplicated(subset=[PERSON_COL, WAVE_COL], keep=False)].sort_values([PERSON_COL, WAVE_COL])
    ...
    use = compute_flag_constant_6m(use)  # flag_constant_6mを先に計算
    use = apply_personal_trend_logic(use)  # trend_refinedで使用
